...
//  this.simVisible_ = false;
    ...
    this.stopProgram = this.createButton_(this.PATH_STOP_, 1, 0, 'MENU_STOP_BRICK');
    this.stopProgram.setAttribute('id', 'stopProgram');
...
//  this.simPause.setAttribute('class', 'robButtonHidden');
    this.saveProgram = this.createButton_(this.PATH_SAVEPROGRAM_, 2, 0, 'MENU_SAVE');
    this.saveProgram.setAttribute('id', 'saveProgram');

    if (this.zoom_) {
        this.zoomVisible_ = false;
        var zoom = this.createButton_(this.PATH_ZOOM_, 3, 0, 'MENU_ZOOM');
        var zoominSvg = this.createButton_(this.PATH_ZOOMIN_, 3, 0, 'MENU_ZOOM_IN');
        var zoomresetSvg = this.createButton_(this.PATH_ZOOMRESET_, 3, 1, 'MENU_ZOOM_RESET');
        var zoomoutSvg = this.createButton_(this.PATH_ZOOMOUT_, 3, 2, 'MENU_ZOOM_OUT');
...
Blockly.RobControls.prototype.hideStopProgram = function() {
    this.stopProgram.setAttribute('visibility', 'hidden');
    this.buttons.forEach(function(button) {
        if (button.posX == 0) {
            button.setAttribute('transform', 'translate(' + ((button.posX + 1) * 50) + ',' + (button.posY * -50) + ')');
        }
    });
};

Blockly.RobControls.prototype.showStopProgram = function() {
    this.stopProgram.setAttribute('visibility', 'visible');
    this.buttons.forEach(function(button) {
        if (button.posX == 0) {
            button.setAttribute('transform', 'translate(' + ((button.posX) * 50) + ',' + (button.posY * -50) + ')');
        }
    });
};
